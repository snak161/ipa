...
def find_datamatrix_candidates(clean, relaxed=False):
...
# === THIRD PASS ===
def third_pass_failed_scans(failed_files, input_dir):
    third_debug_dir = os.path.join(input_dir, "_debug_third_pass")
    os.makedirs(third_debug_dir, exist_ok=True)

    logs_dir = os.path.join(input_dir, "_logs")
    os.makedirs(logs_dir, exist_ok=True)
    timestamp = datetime.datetime.now().strftime("%Y%m%d_%H%M%S")
    log_path = os.path.join(logs_dir, f"third_pass_log_{timestamp}.txt")

    candidates_dir = os.path.join(input_dir, "_candidates_third_pass")
    os.makedirs(candidates_dir, exist_ok=True)
    ...
    robust_variants = [
        {"blur": 3, "block": 21, "C": 5, "method": "otsu"},
        {"blur": 5, "block": 31, "C": 10, "method": "gaussian"},
        {"blur": 3, "block": 25, "C": 2, "method": "clahe"}
    ]

    recovered_map = {}

    with tqdm(total=len(failed_files), desc="Third Pass", ncols=100) as pbar:
        for f in failed_files:
            path = os.path.join(input_dir, f)
            try:
                label_pil = extract_label_image(path)
                ...
                for var in robust_variants:
                    img_bgr, clean = preprocess_label(label_pil, **var)
                    candidates = find_datamatrix_candidates(clean, relaxed=True)
                    all_candidates.extend(candidates)
                    decoded = decode_datamatrix(
                        img_bgr, candidates,
                        debug_path=os.path.join(third_debug_dir, f+"_debug.png"),
                        variant_desc=f"{var}"
                    )
                    if decoded:
                        recovered_map[f] = decoded
    ...
    print(f"\nThird pass finished. Recovered {len(recovered_map)}/{len(failed_files)}.")
    print(f"Debug images saved in: {third_debug_dir}")
    print(f"Candidate crops saved in: {candidates_dir}")
    print(f"Log file for manual review: {log_path}")
...
def main():
    ...
    print(f"\nFound {total} SVS files. Processing first pass...\n")
    ...
    if failed_files:
        ...
        if retry=="y":
            recovered_second = second_pass_failed_scans(failed_files, input_dir)
            failed_after_second = [f for f in failed_files if f not in recovered_second]
            if recovered_second:
                rename_retry = input("Rename recovered SVS files from second pass? (y/n): ").strip().lower()
                if rename_retry=="y":
                    for old_name, decoded_text in recovered_second.items():
                        old_path = os.path.join(input_dir, old_name)
                        new_name_safe = safe_filename(decoded_text)
                        new_path = unique_filename(input_dir, new_name_safe)
                        shutil.move(old_path, new_path)
            ...
            failed_after_second = failed_files
    else:
        failed_after_second = []

    # Third pass
    if failed_after_second:
        retry3 = input("Try third-pass ultra-robust scan for remaining failed files? (y/n): ").strip().lower()
        if retry3=="y":
            recovered_third = third_pass_failed_scans(failed_after_second, input_dir)
            if recovered_third:
                rename_retry3 = input("Rename recovered SVS files from third pass? (y/n): ").strip().lower()
                if rename_retry3=="y":
                    for old_name, decoded_text in recovered_third.items():
                        old_path = os.path.join(input_dir, old_name)
                        new_name_safe = safe_filename(decoded_text)
                        new_path = unique_filename(input_dir, new_name_safe)
                        shutil.move(old_path, new_path)
                    print("✅ Third-pass renaming complete.")
